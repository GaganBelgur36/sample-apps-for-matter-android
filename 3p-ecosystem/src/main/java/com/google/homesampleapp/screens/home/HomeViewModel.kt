...
import com.google.homesampleapp.chip.MatterConstants.MyOnOffAttribute
...
constructor(
    ...
    fun commissionDeviceSucceeded(activityResult: ActivityResult, deviceName: String) {
        ...
        viewModelScope.launch {
            ...
            try {
                ...
                Timber.e(
                    "Adding device [${deviceId}] [${deviceName}] to app's repository failed",
                    e
                )
                _commissionDeviceStatus.postValue(
                    TaskStatus.Failed(
    ...
    fun updateDeviceStateOnForMyOnOff(deviceUiModel: DeviceUiModel, isOn: Boolean) {
        Timber.d("updateDeviceStateOn: Device [${deviceUiModel}]  isOn [${isOn}]")
        viewModelScope.launch {
            Timber.d("Handling real device")
                ...
                val reportCallback =
                    object : SubscriptionHelper.ReportCallbackForDevice(device.deviceId) {
                        override fun onReport(nodeState: NodeState) {
                            ...
                            Timber.d("MyOnOffState [${onOffState}]")
                            if (myOnOffState == null) {
                                Timber.e("onReport(): WARNING -> onOffState is NULL. Ignoring.")
                                return
                            }
                            viewModelScope.launch {
        ...
        viewModelScope.launch {
            while (devicesPeriodicPingEnabled) {
                ...
                devicesList.forEach { device ->
                    ...
                    Timber.d("[device ping] onOff [${isOn}]")

                    var myOn = clustersHelper.getDeviceStateMyOnOffCluster(device.deviceId, 1)
